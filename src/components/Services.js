        ...
        {contService.map((service) => (
          <div className='flex flex-col' key={service.id}>
            <div className='flex flex-col'>
              <div className="max-w-sm rounded overflow-hidden shadow-lg mt-5 mx-5 h-96">
                <img className="w-full h-56 object-cover" src={service.image} alt="Sunset in the mountains" />
                <div className="px-6 py-4 h-40">
                  <div className="font-bold text-xl mb-2 font-serif">{service.name}</div>
                  <p className="text-gray-700 text-base font-serif overflow-ellipsis overflow-hidden h-16">
                    {service.description}
                  </p>
                  <p>{service.price}</p>
                </div>
